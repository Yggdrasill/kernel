 ...
 * ACPI tables.
 *
 * Types 0xFC, 0xFD, 0xFF of the custom types are absolutely not usable
...
static uint32_t mmap_convert_type(uint32_t type)
{
    switch(type) {
...
static uint32_t mmap_map_attribute(uint32_t type, uint32_t attrib)
{
    type = attrib & 0x2 ? MMAP_ATTR_NVS : type;
...
int mmap_transform_map(struct e820_events *event_info, const struct e820_info *src_info)
{
    ...
    if(i < src_info->nr_entries && j == max_nr_events) return -3;

    return 0;
}

/*
 * mmap_sanitize() takes a list of events generated by the E820 transform
 * function above, and will sanitise it into a completely contiguous,
 * non-overlapping memory map. It follows the precedence order outlined earlier
 * and tracks active types that overlap by counting them, then picking the worst
 * one.
 */
...
int mmap_sanitize(struct e820_info *dst_info, const struct e820_events *event_info)
{
    ...
    if(!event_info->nr_events || event_info->nr_events & 1) return -2;
    ...
    /*
     * This may look pointless, but it isn't. The first type may be an unknown
     * type, in which case it should be forcefully converted to MMAP_RESERVED.
     */
...
char *mmap_sanitize_error(int status)
{
    switch(status) {
        ...
        case -2: return "E820san: Empty or odd input event list";
